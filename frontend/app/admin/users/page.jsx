...
export default function AdminUsersPage() {
  ...
  const walletBalance = useMemo(
    () =>
      users.reduce(
        (total, user) =>
          total +
          Number(user?.wallet?.paidPts || 0) +
          Number(user?.wallet?.bonusPts || 0),
        0,
      ),
    [users],
  );
  ...
  const bulkBlockMutation = useBulkMutation(
    ...
    },
    {
      ...
      onError: (mutationError) => {
        setFeedback({
          type: "error",
          message: `封禁所选账号失败：${mutationError.message}`,
        });
      },
    },
  );
  ...
  const bulkUnblockMutation = useBulkMutation(
    ...
    },
    {
      ...
      onError: (mutationError) => {
        setFeedback({
          type: "error",
          message: `恢复所选账号失败：${mutationError.message}`,
        });
      },
    },
  );
  ...
  const bulkDeleteMutation = useBulkMutation(
    ...
    },
    {
      ...
      onError: (mutationError) => {
        setFeedback({
          type: "error",
          message: `删除所选账号失败：${mutationError.message}`,
        });
      },
    },
  );
  ...
  const userBlockMutation = useMutation({
    mutationFn: async ({ userId, blocked }) => {
      ...
      if (!response.ok) {
        throw new Error(
          await readAdminResponseMessage(response, "更新账号状态失败。"),
        );
      }
    ...
    onError: (mutationError) => {
      setFeedback({
        type: "error",
        message: `更新账号状态失败：${mutationError.message}`,
      });
    },
  });
