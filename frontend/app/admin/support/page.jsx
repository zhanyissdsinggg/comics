...
export default function AdminSupportPage() {
  ...
  const bulkDeleteMutation = useBulkDelete("support", {
    ...
    onError: (mutationError) => {
      setFeedback({
        type: "error",
        message: `删除所选工单失败：${mutationError.message}`,
      });
    },
  });
  ...
  const replyTicketMutation = useMutation({
    mutationFn: async ({ ticketId, message }) => {
      ...
      if (!response.ok) {
        throw new Error(
          await readAdminResponseMessage(response, "发送回复失败。"),
        );
      }
    ...
    onError: (mutationError) => {
      setFeedback({
        type: "error",
        message: `发送回复失败：${mutationError.message}`,
      });
    },
  });
  ...
  const closeTicketMutation = useMutation({
    mutationFn: async (ticketId) => {
      const response = await adminFetch(
        `/api/admin/support/${ticketId}/close`,
        {
          method: "PATCH",
        },
      );
      ...
      if (!response.ok) {
        throw new Error(
          await readAdminResponseMessage(response, "关闭工单失败。"),
        );
      }
    ...
    onError: (mutationError) => {
      setFeedback({
        type: "error",
        message: `关闭工单失败：${mutationError.message}`,
      });
    },
  });
