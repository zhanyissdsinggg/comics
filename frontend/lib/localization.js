...
export function normalizeUSDisplayCurrency(currency) {
  const normalizedCurrency = String(currency || US_CURRENCY)
    .trim()
    .toUpperCase();
...
export function formatUSDisplayCurrency(
  amount,
  currency = US_CURRENCY,
  options = {},
) {
...
export function formatUSDisplayCurrencyFromCents(
  amountCents,
  currency = US_CURRENCY,
  options = {},
) {
  if (
    ...
    Number.isNaN(Number(amountCents))
  ) {
    return "$0.00";
  }
