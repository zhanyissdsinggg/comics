      ...
      <div className={`grid gap-4 ${isList ? "lg:grid-cols-[auto,84px,1.6fr,1fr,auto] lg:items-center" : ""}`}>
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
            <span className="rounded-full bg-ios-gray-800 px-2.5 py-1 text-ios-gray-200">{series.type === "novel" ? "Novel" : "Comic"}</span>
            <span className="rounded-full bg-ios-blue/10 px-2.5 py-1 text-ios-blue">{series.status}</span>
            {series.adult ? <span className="rounded-full bg-rose-500/15 px-2.5 py-1 text-rose-300">18+</span> : null}
            {!series.isPublished ? <span className="rounded-full bg-amber-500/15 px-2.5 py-1 text-amber-300">Unpublished</span> : null}
          </div>
          {isEditing ? (
            <div className="space-y-3 rounded-3xl border border-ios-gray-800 bg-ios-gray-950/60 p-4">
              <input value={editDraft?.title || ""} onChange={(event) => onEditDraftChange({ ...editDraft, title: event.target.value })} className="w-full rounded-2xl border border-ios-gray-700 bg-ios-gray-950 px-4 py-3 text-sm text-white outline-none focus:border-ios-blue" placeholder="Series title" />
            ...
            <div className="space-y-1">
              ...
              <p className="line-clamp-2 text-sm text-ios-gray-300">{series.description || "No description yet."}</p>
            </div>
          )}
        </div>
        <div className="grid grid-cols-2 gap-3 rounded-3xl bg-ios-gray-950/50 p-4 text-sm lg:grid-cols-1">
          <div><p className="text-ios-gray-500">Episodes</p><p className="mt-1 font-semibold text-white">{series.episodeCount || 0}</p></div>
          <div><p className="text-ios-gray-500">Updated</p><p className="mt-1 font-semibold text-white">{formatUpdatedAt(series.updatedAt, true)}</p></div>
        </div>
        <div className="flex flex-wrap gap-2 lg:justify-end">
          {isEditing ? (
            <>
              <button type="button" onClick={onSaveEdit} disabled={isSaving} className="rounded-full bg-ios-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{isSaving ? "Saving..." : "Save"}</button>
              <button type="button" onClick={onCancelEdit} disabled={isSaving} className="rounded-full border border-ios-gray-700 px-4 py-2 text-sm font-semibold text-ios-gray-200 transition hover:bg-ios-gray-800 disabled:cursor-not-allowed disabled:opacity-50">Cancel</button>
            </>
          ) : (
            <>
              <button type="button" onClick={() => onOpenDetails(series.id)} className="rounded-full border border-ios-gray-700 px-3 py-2 text-ios-gray-200 transition hover:bg-ios-gray-800" title="Edit details"><Edit size={16} /></button>
              <button type="button" onClick={() => onTogglePublish(series)} className="rounded-full border border-ios-gray-700 px-3 py-2 text-ios-gray-200 transition hover:bg-ios-gray-800" title={series.isPublished ? "Unpublish" : "Publish"}>{series.isPublished ? <EyeOff size={16} /> : <Eye size={16} />}</button>
              <button type="button" onClick={() => onDuplicate(series)} className="rounded-full border border-ios-gray-700 px-3 py-2 text-ios-gray-200 transition hover:bg-ios-gray-800" title="Duplicate"><Copy size={16} /></button>
              <button type="button" onClick={() => onDelete(series)} className="rounded-full border border-rose-500/30 px-3 py-2 text-rose-300 transition hover:bg-rose-500/10" title="Delete"><Trash2 size={16} /></button>
            </>
          )}
        </div>
      </div>
...
export default function AdminSeriesPageNew() {
  ...
  const loadSeries = useCallback(async () => {
    ...
    if (response.ok) {
      ...
      setSeriesList([]);
      setFeedback({ type: "error", message: response.error || "Failed to load series." });
    }
    setLoading(false);
  ...
  const handleSaveEdit = async (seriesId) => {
    ...
    if (response.ok) {
      ...
      setFeedback({ type: "success", message: "Series updated." });
      handleCancelEdit();
    } else {
      setFeedback({ type: "error", message: response.error || "Failed to save changes." });
    }
  ...
  const handleTogglePublish = async (series) => {
    ...
    if (response.ok) {
      ...
      setFeedback({ type: "success", message: nextPublished ? "Series published." : "Series unpublished." });
    } else {
      setFeedback({ type: "error", message: response.error || "Failed to update publish state." });
    }
  };
  ...
  const uploadCoverImage = async (file) => {
    ...
    if (!response.ok || !response.data?.url) throw new Error(response.error || "Failed to upload cover image.");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.error || "Failed to create the series.");
      setFeedback({ type: "success", message: "Series created successfully." });
      setShowCreateModal(false);
      resetCreateForm();
      await loadSeries();
    } catch (error) {
      setFeedback({ type: "error", message: error instanceof Error ? error.message : "Failed to create the series." });
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleDelete = (series) => setConfirmDialog({
    isOpen: true,
    title: "Delete series",
    message: `Delete ${series.title}? This action cannot be undone.`,
    variant: "danger",
    onConfirm: async () => {
      ...
      if (response.ok) {
        ...
        setFeedback({ type: "success", message: "Series deleted." });
      } else {
        setFeedback({ type: "error", message: response.error || "Failed to delete the series." });
      }
    },
  });
  ...
  const handleDuplicate = async () => {
    ...
    if (response.ok) {
      setFeedback({ type: "success", message: "Series duplicated." });
      setDuplicateDialog({ isOpen: false, series: null, newId: "" });
      await loadSeries();
    } else {
      setFeedback({ type: "error", message: response.error || "Failed to duplicate the series." });
    }
  ...
  const updatePublishStateForSelection = async (nextPublished) => {
    ...
    setFeedback({ type: "success", message: nextPublished ? "Selected series published." : "Selected series unpublished." });
  };
  ...
  const handleBulkDelete = async () => setConfirmDialog({
    ...
    message: `Delete ${selectedSeries.length} selected series? This action cannot be undone.`,
    variant: "danger",
    onConfirm: async () => {
      ...
      setSelectedSeries([]);
      setFeedback({ type: "success", message: "Selected series deleted." });
    },
  });
  ...
  if (isLoading || loading) return <section className="rounded-5xl border border-ios-gray-800 bg-neutral-900/60 p-8 text-sm text-ios-gray-300">Loading series...</section>;
  if (!isAuthenticated) return <section className="rounded-5xl border border-dashed border-ios-gray-700 bg-neutral-900/60 p-10 text-center text-sm text-ios-gray-300">Admin access is required.</section>;
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-5xl border border-ios-gray-800 bg-gradient-to-br from-ios-blue/10 to-ios-purple/10 p-6 shadow-ios">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-white">Series management</h2>
            <p className="text-sm text-ios-gray-300">Search, edit, duplicate, and organize your catalog without leaving the dashboard.</p>
          </div>
          <button type="button" onClick={() => setShowCreateModal(true)} className="inline-flex items-center gap-2 rounded-full bg-ios-blue px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-500"><Plus size={16} /><span>Add series</span></button>
        </div>
      </section>
      ...
      <section className="rounded-5xl border border-ios-gray-800 bg-neutral-900/60 p-5 shadow-ios">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-1 flex-wrap items-center gap-2 xl:justify-end">
            <label className="flex min-w-[260px] flex-1 items-center gap-3 rounded-full border border-ios-gray-800 bg-ios-gray-950/80 px-4 py-3 xl:max-w-md"><Search size={16} className="text-ios-gray-500" /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search series title or ID..." className="w-full bg-transparent text-sm text-white outline-none placeholder:text-ios-gray-500" /></label>
            <AdvancedFilters filters={advancedFilters} onFiltersChange={setAdvancedFilters} />
            <button type="button" onClick={handleToggleSelectAll} disabled={filteredSeries.length === 0} className="rounded-full border border-ios-gray-800 bg-ios-gray-950/80 px-4 py-3 text-sm font-semibold text-ios-gray-200 transition hover:bg-ios-gray-900 disabled:cursor-not-allowed disabled:opacity-50">{allVisibleSelected ? "Clear selection" : "Select all"}</button>
            <div className="flex items-center overflow-hidden rounded-full border border-ios-gray-800 bg-ios-gray-950/80">
              <button type="button" onClick={() => setViewMode("grid")} className={`px-4 py-3 transition ${viewMode === "grid" ? "bg-ios-blue text-white" : "text-ios-gray-300 hover:bg-ios-gray-900"}`} title="Grid view"><Grid size={16} /></button>
              <button type="button" onClick={() => setViewMode("list")} className={`px-4 py-3 transition ${viewMode === "list" ? "bg-ios-blue text-white" : "text-ios-gray-300 hover:bg-ios-gray-900"}`} title="List view"><List size={16} /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      {filteredSeries.length === 0 ? (
        <section className="rounded-5xl border border-dashed border-ios-gray-700 bg-neutral-900/60 p-12 text-center"><ImageIcon size={36} className="mx-auto text-ios-gray-500" /><h3 className="mt-4 text-lg font-semibold text-white">No series found</h3><p className="mt-2 text-sm text-ios-gray-400">{searchQuery || typeFilter !== "all" || advancedFilters.status !== "all" || advancedFilters.publishStatus !== "all" || advancedFilters.adultContent !== "all" ? "Try changing filters or search keywords." : "Create your first series from the add button."}</p></section>
      ) : (
          ...
          <div className="w-full max-w-2xl rounded-5xl border border-ios-gray-800 bg-neutral-900 p-6 shadow-ios-xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-ios-gray-400">Create a new catalog entry with an optional cover upload.</p>
              </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr,1.1fr]">
              <div>
                <label className="mb-2 block text-sm font-semibold text-ios-gray-300">Cover image</label>
                <div onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={(event) => { event.preventDefault(); setIsDragging(false); handleCoverInput(event.dataTransfer.files?.[0]); }} className={`rounded-5xl border border-dashed p-4 transition ${isDragging ? "border-ios-blue bg-ios-blue/10" : "border-ios-gray-700 bg-ios-gray-950/40"}`}>
                  {createForm.coverPreviewUrl ? (
                    <div className="space-y-3">
                      <img src={createForm.coverPreviewUrl} alt="Cover preview" className="aspect-[2/3] w-full rounded-4xl object-cover" />
                      <button type="button" onClick={() => setCreateForm((current) => { revokeObjectUrl(current.coverPreviewUrl); return { ...current, coverFile: null, coverPreviewUrl: "" }; })} className="w-full rounded-full border border-ios-gray-700 px-4 py-2 text-sm font-semibold text-ios-gray-200 transition hover:bg-ios-gray-800">Remove cover</button>
                    </div>
                  ) : (
                    <label className="flex min-h-[320px] cursor-pointer flex-col items-center justify-center gap-3 rounded-4xl border border-ios-gray-800 bg-ios-gray-950/70 px-6 text-center text-ios-gray-400">
                      ...
                      <div><p className="text-sm font-semibold text-white">Drag an image here or click to upload</p><p className="mt-1 text-xs text-ios-gray-500">Supports JPG, PNG, and GIF. Max 10MB.</p></div>
              ...
              <div className="space-y-4">
                <label className="block space-y-2">
                  <span className="text-sm font-semibold text-ios-gray-300">Series title *</span>
                  <input value={createForm.title} onChange={(event) => setCreateForm((current) => ({ ...current, title: event.target.value }))} placeholder="For example: Midnight Contract" className="w-full rounded-3xl border border-ios-gray-700 bg-ios-gray-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-ios-blue" />
                  <span className="text-xs text-ios-gray-500">A unique ID is generated automatically when the series is created.</span>
                </label>

                <div className="space-y-2">
                  <span className="text-sm font-semibold text-ios-gray-300">Series type</span>
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setCreateForm((current) => ({ ...current, type: "comic" }))} className={`rounded-3xl px-4 py-3 text-sm font-semibold transition ${createForm.type === "comic" ? "bg-ios-blue text-white" : "bg-ios-gray-950/70 text-ios-gray-300 hover:bg-ios-gray-900"}`}>Comic</button>
                    <button type="button" onClick={() => setCreateForm((current) => ({ ...current, type: "novel" }))} className={`rounded-3xl px-4 py-3 text-sm font-semibold transition ${createForm.type === "novel" ? "bg-ios-purple text-white" : "bg-ios-gray-950/70 text-ios-gray-300 hover:bg-ios-gray-900"}`}>Novel</button>
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={closeCreateModal} className="flex-1 rounded-full border border-ios-gray-700 px-4 py-3 text-sm font-semibold text-ios-gray-200 transition hover:bg-ios-gray-800">Cancel</button>
                  <button type="button" onClick={handleCreate} disabled={isCreating} className="flex-1 rounded-full bg-ios-blue px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{isCreating ? "Creating..." : "Create"}</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {duplicateDialog.isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setDuplicateDialog({ isOpen: false, series: null, newId: "" })}>
          <div className="w-full max-w-lg rounded-5xl border border-ios-gray-800 bg-neutral-900 p-6 shadow-ios-xl" onClick={(event) => event.stopPropagation()}>
            <h3 className="text-xl font-bold text-white">Duplicate series</h3>
            <p className="mt-1 text-sm text-ios-gray-400">Create a new series from the selected item.</p>
            <label className="mt-5 block space-y-2">
              <span className="text-sm font-semibold text-ios-gray-300">New series ID *</span>
              <input value={duplicateDialog.newId} onChange={(event) => setDuplicateDialog((current) => ({ ...current, newId: event.target.value }))} placeholder="Enter a new series ID" className="w-full rounded-3xl border border-ios-gray-700 bg-ios-gray-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-ios-blue" />
            </label>
            <div className="mt-6 flex gap-3">
              <button type="button" onClick={() => setDuplicateDialog({ isOpen: false, series: null, newId: "" })} className="flex-1 rounded-full border border-ios-gray-700 px-4 py-3 text-sm font-semibold text-ios-gray-200 transition hover:bg-ios-gray-800">Cancel</button>
              <button type="button" onClick={handleDuplicate} disabled={isDuplicating} className="flex-1 rounded-full bg-ios-blue px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">{isDuplicating ? "Duplicating..." : "Duplicate"}</button>
            </div>
          </div>
        </div>
      ) : null}
