    ...
    >
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
        <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4 shadow-[0_12px_28px_rgba(15,23,42,0.032)] ring-1 ring-black/[0.02]">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">时间视图</p>
          <p className="mt-2 text-sm text-slate-600">先切换统计窗口，再看收入、渠道和活动表现，避免把不同周期混着比。</p>
        ...
        <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4 shadow-[0_12px_28px_rgba(15,23,42,0.032)] ring-1 ring-black/[0.02]">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">自定义日期</p>
  ...
  return (
    <AdminDataState isLoading={overviewLoading} hasData={hasOverviewData} emptyMessage={EMPTY_MESSAGE} wrap={false}>
      <div className="space-y-6">
        {stats ? (
          <AdminPageSection title="收入总览" description="先看收入、退款和订单质量。" eyebrow="总体情况">
        ...
        {userValue ? (
          <AdminPageSection title="付费读者层级" description="按累计消费看读者价值结构。" eyebrow="用户价值">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {orderStatus ? (
          <AdminPageSection title="订单结果结构" description="看清订单健康度。" eyebrow="订单健康">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
...
export function RevenueTrendSection({ trendLoading, trend }) {
  return (
    <AdminDataState isLoading={trendLoading} hasData={trend.length > 0} emptyMessage={EMPTY_MESSAGE} wrap={false}>
      <AdminPageSection title="收入趋势" description="按天看收入和支付订单变化。" eyebrow="趋势变化">
        <AdminDataTable>
...
export function RevenueChannelsSection({ channelsLoading, channels }) {
  return (
    <AdminDataState isLoading={channelsLoading} hasData={channels.length > 0} emptyMessage={EMPTY_MESSAGE} wrap={false}>
      <AdminPageSection title="渠道表现" description="比较不同渠道带来的收入。" eyebrow="渠道对比">
        <AdminDataTable>
          <table className="w-full text-sm">
            <AdminTableHeader>
              <tr>
