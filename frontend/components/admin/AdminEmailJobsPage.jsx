...
export default function AdminEmailJobsPage() {
  ...
  const failedCount = useMemo(
    () =>
      jobs.filter((job) => String(job.status || "").toUpperCase() === "FAILED")
        .length,
    [jobs],
  );
  ...
  const handleRetry = async (jobId) => {
    ...
    try {
      const response = await adminPost("/api/admin/email/jobs/retry", {
        jobId,
      });
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => loadData()}
              data-testid="admin-email-jobs-refresh"
            >
              刷新
            </Button>
            <Button
                ...
                {jobs.map((job) => {
                  ...
                  return (
                    <AdminTableRow key={job.id}>
                      ...
                      <td className="px-4 py-4 text-slate-600">
                        {formatAttemptAt(job.lastAttemptAt)}
                      </td>
