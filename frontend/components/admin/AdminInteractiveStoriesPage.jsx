...
export default function AdminInteractiveStoriesPage() {
  ...
  const selectedNode = useMemo(() => nodes.find((item) => item.id === selectedNodeId) || nodes[0] || null, [nodes, selectedNodeId]);
  const selectedNodeForm = selectedNode ? nodeForms[selectedNode.id] || emptyNode() : emptyNode();
  const stateKeys = useMemo(() => summarizeStateKeys(story.initialStateText), [story.initialStateText]);
  const storyDirty = useMemo(() => {
    if (!detail) {
      return Boolean(story.slug || story.title || story.seriesId || story.description || story.baseContext || story.initialStateText);
    }
  ...
  const nodeDirtyById = useMemo(() => {
    ...
    for (const node of nodes) {
      next[node.id] = !compareFormState(nodeForms[node.id] || emptyNode(), mapNodeToForm(node));
    }
    return next;
  }, [nodeForms, nodes]);
  const choiceDirtyById = useMemo(() => {
    ...
    for (const node of nodes) {
      for (const choice of node.choices || []) {
        next[choice.id] = !compareFormState(choiceForms[choice.id] || emptyChoice(), mapChoiceToForm(choice));
      }
    }
    return next;
  }, [choiceForms, nodes]);
  const filteredStories = useMemo(() => {
    ...
    return stories.filter((item) =>
      [item.title, item.slug, item.series?.title].filter(Boolean).join(" ").toLowerCase().includes(q),
    );
  }, [stories, storyQuery]);
  const filteredNodes = useMemo(() => {
    const q = nodeQuery.trim().toLowerCase();
    return nodes.filter((node) => {
      const matchesQuery = !q || [node.title, node.nodeKey, ...(node.choices || []).map((choice) => choice.label)]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(q);
      ...
      switch (nodeFilterMode) {
        ...
        case "empty":
          return (node.choices?.length || 0) === 0;
        case "dirty":
          return Boolean(nodeDirtyById[node.id]) || (node.choices || []).some((choice) => choiceDirtyById[choice.id]);
        default:
          return true;
      }
    });
  }, [choiceDirtyById, detail?.initialNodeId, nodeDirtyById, nodeFilterMode, nodeQuery, nodes]);
  const selectedNodeDirty = selectedNode ? Boolean(nodeDirtyById[selectedNode.id]) : false;
  const selectedChoiceDirtyCount = useMemo(() => {
    if (!selectedNode) return 0;
    return (selectedNode.choices || []).filter((choice) => choiceDirtyById[choice.id]).length;
  }, [choiceDirtyById, selectedNode]);
  const dirtyNodeCount = useMemo(
    () =>
      nodes.filter((node) => Boolean(nodeDirtyById[node.id]) || (node.choices || []).some((choice) => choiceDirtyById[choice.id]))
        .length,
  ...
  const loadStories = useCallback(async () => {
    ...
    setStories(list);
    setSelectedStoryId((current) => (list.some((item) => item.id === current) ? current : list[0]?.id || ""));
    setLoading(false);
  }, []);
  ...
  const loadDetail = useCallback(async (id) => {
    ...
    setSelectedNodeId((current) => (next?.nodes?.some((item) => item.id === current) ? current : next?.nodes?.[0]?.id || ""));
  ...
  async function saveStory() {
    ...
    setBusy(true);
    const resp = await adminPatch(`/api/admin/interactive-stories/${selectedStoryId}`, { story: payload });
    setBusy(false);
    if (!resp.ok) {
  ...
  async function createStory() {
    ...
    setBusy(true);
    const resp = await adminPost("/api/admin/interactive-stories", { story: payload });
    setBusy(false);
    if (!resp.ok) {
  ...
  async function publishStory(publish) {
    if (!selectedStoryId) return;
    setBusy(true);
    const resp = await adminPost(`/api/admin/interactive-stories/${selectedStoryId}/publish`, { publish });
    setBusy(false);
    if (!resp.ok) {
      setFeedback({ type: "error", message: msg(resp, publish ? "发布失败，请先修复校验问题" : "下线失败") });
      return;
    }
    setFeedback({ type: "success", message: publish ? "故事已发布到前台" : "故事已从前台下线" });
    await loadStories();
    await loadDetail(selectedStoryId);
  }
  ...
  async function saveNode(nodeId) {
    ...
    const resp = await adminPatch(`/api/admin/interactive-stories/nodes/${nodeId}`, { node: payload });
    if (!resp.ok) {
  ...
  async function saveChoice(choiceId) {
    ...
    const resp = await adminPatch(`/api/admin/interactive-stories/choices/${choiceId}`, { choice: payload });
    if (!resp.ok) {
  ...
  async function addChoice(nodeId) {
    ...
    const resp = await adminPost(`/api/admin/interactive-stories/nodes/${nodeId}/choices`, { choice: payload });
    if (!resp.ok) {
  ...
  function openRemoveStoryConfirm() {
    ...
    setConfirmDialog({
      isOpen: true,
      title: "删除互动故事",
      message: "确认删除当前故事吗？删除后节点与选项会一起移除，已经保存的读者进度也会失去对应内容。",
      confirmText: "删除故事",
      variant: "danger",
      onConfirm: () => {
        void removeStory();
  ...
  function openRemoveNodeConfirm(nodeId) {
    ...
    setConfirmDialog({
      isOpen: true,
      title: "删除节点",
      message: "确认删除这个节点吗？系统会自动处理受影响的分支引用，但这个节点本身的结构和文案不会保留。",
      confirmText: "删除节点",
      variant: "danger",
      onConfirm: () => {
  ...
  async function exportStory() {
    ...
    exportJson(`${payload?.story?.slug || selectedStoryId}-interactive-story.json`, payload);
  ...
  async function importStory() {
    ...
    setBusy(true);
    const resp = await adminPost("/api/admin/interactive-stories/import", { mode: importMode, payload });
    setBusy(false);
    if (!resp.ok) {
  ...
  const selectedStorySummary = detail
    ? [
        ...
        { label: "起始节点", value: nodes.find((item) => item.id === detail.initialNodeId)?.title || "未设置" },
        { label: "AI 生成", value: detail.aiEnabled ? "开启" : "关闭" },
        { label: "状态键", value: stateKeys.length > 0 ? stateKeys.join(" / ") : "未定义" },
      ]
    : [];
    ...
    <AdminShell
      ...
      actions={(
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => loadDetail(selectedStoryId)} disabled={!selectedStoryId}>
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <AdminMetricCard label="节点总数" value={nodes.length} detail="当前图谱里的剧情节点数量" />
          <AdminMetricCard label="分支总数" value={nodes.reduce((sum, node) => sum + (node.choices?.length || 0), 0)} detail="所有节点下的选项总和" />
          <AdminMetricCard label="结局节点" value={nodes.filter((node) => node.isEnding).length} detail="已标记为结局的节点数量" />
          <AdminMetricCard
            label="发布校验"
            value={validation?.ok ? "可发布" : "待修复"}
            detail={validation ? `错误 ${validation.errors} / 警告 ${validation.warnings}` : "先运行一次结构校验"}
            tone={validation?.ok ? "accent" : "default"}
          />
        </div>

        <div className="grid gap-6 xl:grid-cols-[320px,minmax(0,1fr)]">
          <AdminPageSection title="故事库" description="切换已有故事，或直接开一个新的互动稿。">
          ...
          <div className="space-y-6">
            <AdminPageSection
              title={selectedStoryId ? (detail?.title || "故事工作区") : "开始新故事"}
              description={selectedStoryId ? "先把世界观和起始状态定好，再去排节点与分支。" : "填好基础资料后就能创建故事，再进入节点编排。"}
            ...
            >
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_360px]">
                  <div className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      <AdminFormField label="故事 slug" helperText="用于后台识别和导入导出命名。">
                        <input
                          value={story.slug}
                          onChange={(event) => setStory((current) => ({ ...current, slug: event.target.value }))}
                          className={adminInputClassName}
                          placeholder="midnight-archive"
                        />
                      </AdminFormField>
                      <AdminFormField label="故事标题" helperText="面向运营与作者的主标题。">
                        <input
                          value={story.title}
                      ...
                      <AdminFormField label="绑定作品 ID" helperText="如果要挂到现有小说详情页，这里填 seriesId。">
                        <input
                          value={story.seriesId}
                          onChange={(event) => setStory((current) => ({ ...current, seriesId: event.target.value }))}
                          className={adminInputClassName}
                          placeholder="series-011"
                        />
                      </AdminFormField>
                      <AdminFormField label="一句话简介" helperText="给后台同事快速理解这篇互动稿的定位。">
                        <input
                          value={story.description}
                          onChange={(event) => setStory((current) => ({ ...current, description: event.target.value }))}
                          className={adminInputClassName}
                    ...
                    <AdminFormField label="世界观与主线背景" helperText="这里写不可跑偏的设定、角色关系、叙事边界，AI 会把它当作母设定。">
                      <textarea
                        rows={10}
                        value={story.baseContext}
                        onChange={(event) => setStory((current) => ({ ...current, baseContext: event.target.value }))}
                    ...
                    >
                      <textarea
                        ...
                        onChange={(event) => setStory((current) => ({ ...current, initialStateText: event.target.value }))}
                        className={adminTextareaClassName}
                    ...
                    <div className="flex flex-wrap gap-4">
                      <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                        <input
                          ...
                          onChange={(event) => setStory((current) => ({ ...current, aiEnabled: event.target.checked }))}
                      ...
                      <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                        <input
                          ...
                          onChange={(event) => setStory((current) => ({ ...current, isPublished: event.target.checked }))}
                          className={adminCheckboxClassName}
                        />
                        发布状态仅作参考显示
                      </label>
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      <p className="text-sm font-semibold text-slate-950">
                        {validation?.ok ? "当前图谱可发布" : "当前图谱还没达到发布标准"}
                      </p>
                      <p className="mt-1 text-xs leading-5 text-slate-600">
                        {validation ? `错误 ${validation.errors}，警告 ${validation.warnings}` : "建议先保存故事，再运行一次结构校验。"}
                      </p>
                    </div>
                    ...
                    {selectedStorySummary.length > 0 ? (
                      <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4">
                        <div className="flex items-center justify-between gap-3">
                          <p className="text-sm font-semibold text-slate-950">当前故事概览</p>
                          {storyDirty ? <AdminBadge tone="warning">有未保存修改</AdminBadge> : <AdminBadge tone="success">已同步</AdminBadge>}
                        </div>
                        <div className="mt-3 space-y-3">
                          {selectedStorySummary.map((item) => (
                            <div key={item.label} className="flex items-center justify-between gap-4 border-b border-[color:var(--gush-border)] pb-3 last:border-b-0 last:pb-0">
                    ...
                    <div className="flex flex-col gap-2">
                      <Button className="w-full" onClick={selectedStoryId ? saveStory : createStory} disabled={busy}>
                      ...
                      {selectedStoryId ? (
                        <Button className="w-full" onClick={() => publishStory(true)} disabled={busy}>
                          <CheckCircle2 className="size-4" />
                          发布到前台
                        </Button>
                      ) : null}
                      {selectedStoryId ? (
                        <Button className="w-full" variant="secondary" onClick={() => publishStory(false)} disabled={busy}>
                          下线互动入口
                        </Button>
                      ) : null}
                      {selectedStoryId ? (
                        <Button className="w-full" variant="destructive" onClick={openRemoveStoryConfirm}>
              ...
              {activeTab === "json" ? (
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
                  <div className="space-y-4">
                    <AdminFormField label="导入模式" helperText="新建模式不会覆盖现有稿件；替换模式会清空原有节点后重建。">
                      <select value={importMode} onChange={(event) => setImportMode(event.target.value)} className={adminSelectClassName}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4 text-sm text-slate-600">
                      ...
                      <div className="mt-3 space-y-2 leading-6">
                        ...
                        <p>节点层：nodeKey、title、baseContext、basePrompt、fallbackText。</p>
                        <p>分支层：choiceKey、label、跳转节点、条件、状态变更。</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="space-y-3 text-sm leading-6 text-slate-600">
                  <div className="flex items-start gap-3 rounded-[18px] border border-[color:var(--gush-border)] bg-white px-4 py-3">
                    ...
                    <p>故事背景要写在母设定里，节点只负责当前场景，不要把总设定散得到处都是。</p>
                  </div>
                  <div className="flex items-start gap-3 rounded-[18px] border border-[color:var(--gush-border)] bg-white px-4 py-3">
                    <GitBranch className="mt-0.5 size-4 shrink-0 text-slate-500" />
                    <p>节点分支最好保持清晰的心理差异或信息差，不要做文案不一样但结果一样的假分支。</p>
                  </div>
                  <div className="flex items-start gap-3 rounded-[18px] border border-[color:var(--gush-border)] bg-white px-4 py-3">
                    <ListChecks className="mt-0.5 size-4 shrink-0 text-slate-500" />
                    <p>状态变更和标记条件尽量写短、写准，后面查问题的时候才不会把自己坑死。</p>
                  </div>
                </div>
              </AdminPageSection>
            </div>
...
function InteractiveStoryNodesTab({
  ...
  const selectedNodeTargets = useMemo(() => {
    ...
    const targetIds = Array.from(new Set((selectedNode.choices || []).map((choice) => choice.targetNodeId).filter(Boolean)));
    return targetIds
  ...
  const newNodeReady = Boolean(nodeDraft.nodeKey.trim() && nodeDraft.title.trim());

  return (
    <div className="space-y-4">
                  <div className="grid gap-3 xl:grid-cols-4">
                    <div className="rounded-[22px] border border-[color:var(--gush-border)] bg-white px-4 py-4">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">当前焦点</p>
                      <p className="mt-2 text-sm font-semibold text-slate-950">{selectedNode?.title || "还没选节点"}</p>
                      <p className="mt-1 text-xs text-slate-500">{selectedNode?.nodeKey || "先从节点地图里选中一个节点"}</p>
                    </div>
                    <div className="rounded-[22px] border border-[color:var(--gush-border)] bg-white px-4 py-4">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">待保存</p>
                      <p className="mt-2 text-sm font-semibold text-slate-950">{dirtyNodeCount} 个节点</p>
                      <p className="mt-1 text-xs text-slate-500">
                        {selectedNodeDirty || selectedChoiceDirtyCount > 0 ? "当前选中节点也有未保存改动" : "当前节点区暂时是干净的"}
                      </p>
                    </div>
                    <div className="rounded-[22px] border border-[color:var(--gush-border)] bg-white px-4 py-4">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">分支情况</p>
                      <p className="mt-2 text-sm font-semibold text-slate-950">{selectedNodeChoiceCount} 个选项</p>
                      <p className="mt-1 text-xs text-slate-500">
                        {selectedNode ? (selectedNodeHasJumpTargets ? `可跳到 ${selectedNodeTargets.length} 个目标节点` : "还没有有效跳转目标") : "选中节点后会显示分支概览"}
                      </p>
                    </div>
                    <div className="rounded-[22px] border border-[color:var(--gush-border)] bg-white px-4 py-4">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">新增节点</p>
                      <p className="mt-2 text-sm font-semibold text-slate-950">{newNodeReady ? "可以直接创建" : "先补 Key 和标题"}</p>
                      <p className="mt-1 text-xs text-slate-500">基础信息先填够，再往下补背景、状态和 AI 提示词。</p>
                    </div>
                  </div>
                  <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
                    <div className="space-y-4">
                      <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-[color:var(--gush-page-bg-muted)]/60 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <h3 className="text-sm font-semibold text-slate-950">节点地图</h3>
                          ...
                          {detail?.initialNodeId ? (
                            <AdminBadge tone="accent">
                              起始节点：{nodes.find((item) => item.id === detail.initialNodeId)?.title || "未命名节点"}
                            </AdminBadge>
                          ) : (
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <AdminBadge>{filteredNodes.length} / {nodes.length} 个节点</AdminBadge>
                          <AdminBadge tone={dirtyNodeCount > 0 ? "warning" : "default"}>{dirtyNodeCount} 个节点待保存</AdminBadge>
                          <AdminBadge>{nodes.filter((node) => (node.choices?.length || 0) === 0).length} 个空分支</AdminBadge>
                        </div>
                                ...
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                      ...
                                      <p className="mt-1 truncate text-xs text-slate-500">{node.nodeKey}</p>
                                    </div>
                                    <div className="flex flex-col items-end gap-2">
                                      <AdminBadge tone={detail?.initialNodeId === node.id ? "accent" : node.isEnding ? "warning" : "default"}>
                                        {detail?.initialNodeId === node.id ? "起点" : node.isEnding ? "结局" : `${outgoingChoices.length} 个选项`}
                                      </AdminBadge>
                                      {nodeDirtyById[node.id] || outgoingChoices.some((choice) => choiceDirtyById[choice.id]) ? (
                                        <AdminBadge tone="warning">未保存</AdminBadge>
                                      ) : null}
                                    </div>
                                  </div>
                                  <div className="mt-4 space-y-2">
                                    {outgoingChoices.length > 0 ? outgoingChoices.slice(0, 3).map((choice) => {
                                      const targetNode = nodes.find((item) => item.id === choice.targetNodeId);
                                      return (
                                        <div key={choice.id} className="flex items-center gap-2 text-xs text-slate-500">
                                          <ArrowRight className="size-3 shrink-0" />
                                          <span className="truncate">{choice.label}</span>
                                          <span className="truncate text-slate-400">
                                            {targetNode ? `→ ${targetNode.title}` : "→ 留在当前节点"}
                                          </span>
                                        </div>
                                      );
                                    }) : (
                                      <p className="text-xs text-slate-400">这个节点还没有选项。</p>
                                    )}
                                  </div>
                                </button>
                              );
                      ...
                      <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h3 className="text-sm font-semibold text-slate-950">新增节点</h3>
                            <p className="mt-1 text-sm leading-6 text-slate-600">
                          ...
                          <div className="mt-4 space-y-4">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                              <AdminFormField label="节点 Key">
                                ...
                                onChange={(event) => setNodeDraft((current) => ({ ...current, nodeKey: event.target.value }))}
                                className={adminInputClassName}
                            ...
                            <AdminFormField label="排序">
                              <input
                                ...
                                onChange={(event) => setNodeDraft((current) => ({ ...current, sortOrder: event.target.value }))}
                                className={adminInputClassName}
                              />
                            </AdminFormField>
                            <AdminFormField label="回退文案" helperText="AI 生成失败时返回给读者的保底文本。">
                              <input
                                value={nodeDraft.fallbackText}
                                onChange={(event) => setNodeDraft((current) => ({ ...current, fallbackText: event.target.value }))}
                                className={adminInputClassName}
                          ...
                          <AdminFormField label="节点剧情背景">
                            <textarea
                              ...
                              onChange={(event) => setNodeDraft((current) => ({ ...current, baseContext: event.target.value }))}
                              className={adminTextareaClassName}
                          ...
                          <AdminFormField label="节点 AI 提示词">
                            <textarea
                              ...
                              onChange={(event) => setNodeDraft((current) => ({ ...current, basePrompt: event.target.value }))}
                              className={adminTextareaClassName}
                              placeholder="告诉 AI 这一段的情绪、镜头感和必须提到的关键点。"
                            />
                          </AdminFormField>
                          <div className="grid gap-4 md:grid-cols-2">
                            <AdminFormField label="必需标记" helperText="逗号或换行分隔，例如 met_editor, unlocked_note。">
                              <textarea
                                rows={4}
                                value={nodeDraft.requiredFlagsText}
                                onChange={(event) => setNodeDraft((current) => ({ ...current, requiredFlagsText: event.target.value }))}
                                className={adminTextareaClassName}
                                placeholder="clue_found, met_editor"
                              />
                            </AdminFormField>
                            <AdminFormField label="屏蔽标记" helperText="命中这些标记时，该节点不应被访问。">
                              <textarea
                                rows={4}
                                value={nodeDraft.blockedFlagsText}
                                onChange={(event) => setNodeDraft((current) => ({ ...current, blockedFlagsText: event.target.value }))}
                                className={adminTextareaClassName}
                                placeholder="bad_ending"
                              />
                            </AdminFormField>
                          </div>
                          <AdminFormField label="节点状态变更 JSON" helperText='例如 {"trust": 1, "flagsAdd": ["met_editor"]}。'>
                            <textarea
                              rows={5}
                              value={nodeDraft.stateEffectsText}
                              onChange={(event) => setNodeDraft((current) => ({ ...current, stateEffectsText: event.target.value }))}
                              className={adminTextareaClassName}
                          ...
                          <div className="flex flex-wrap gap-4">
                            <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                              <input
                                ...
                                onChange={(event) => setNodeDraft((current) => ({ ...current, isEnding: event.target.checked }))}
                                className={adminCheckboxClassName}
                              />
                              结局节点
                            </label>
                            <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                              <input
                                ...
                                onChange={(event) => setNodeDraft((current) => ({ ...current, aiEnabled: event.target.checked }))}
                                className={adminCheckboxClassName}
                              />
                              节点启用 AI
                            </label>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h3 className="text-sm font-semibold text-slate-950">节点编辑器</h3>
                            <p className="mt-1 text-sm leading-6 text-slate-600">
                          ...
                          <div className="flex flex-wrap justify-end gap-2">
                            {selectedNode ? (
                              <AdminBadge tone={detail?.initialNodeId === selectedNode.id ? "accent" : selectedNode.isEnding ? "warning" : "default"}>
                                {detail?.initialNodeId === selectedNode.id ? "起始节点" : selectedNode.isEnding ? "结局节点" : "普通节点"}
                              </AdminBadge>
                            ) : null}
                            {selectedNode && selectedNodeDirty ? <AdminBadge tone="warning">节点未保存</AdminBadge> : null}
                            {selectedChoiceDirtyCount > 0 ? <AdminBadge tone="warning">{selectedChoiceDirtyCount} 个分支待保存</AdminBadge> : null}
                          </div>
                        </div>

                        {!selectedNode ? (
                          <p className="mt-4 text-sm leading-6 text-slate-500">先从左边节点地图里选中一个节点。</p>
                        ) : (
                          <div className="mt-4 space-y-4">
                            <div className="rounded-[20px] border border-[color:var(--gush-border)] bg-[color:var(--gush-page-bg-muted)]/80 p-4">
                              <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                              <AdminFormField label="节点 Key">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, nodeKey: event.target.value },
                                  }))}
                                  className={adminInputClassName}
                                />
                              </AdminFormField>
                              <AdminFormField label="节点标题">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, title: event.target.value },
                                  }))}
                                  className={adminInputClassName}
                                />
                              </AdminFormField>
                              <AdminFormField label="排序">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, sortOrder: event.target.value },
                                  }))}
                                  className={adminInputClassName}
                                />
                              </AdminFormField>
                              <AdminFormField label="回退文案">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, fallbackText: event.target.value },
                                  }))}
                                  className={adminInputClassName}
                                />
                              </AdminFormField>
                            ...
                            <AdminFormField label="节点剧情背景">
                              <textarea
                                ...
                                onChange={(event) => setNodeForms((current) => ({
                                  ...current,
                                  [selectedNode.id]: { ...selectedNodeForm, baseContext: event.target.value },
                                }))}
                                className={adminTextareaClassName}
                              />
                            </AdminFormField>
                            <AdminFormField label="节点 AI 提示词">
                              <textarea
                                ...
                                onChange={(event) => setNodeForms((current) => ({
                                  ...current,
                                  [selectedNode.id]: { ...selectedNodeForm, basePrompt: event.target.value },
                                }))}
                                className={adminTextareaClassName}
                              />
                            </AdminFormField>
                            <div className="grid gap-4 md:grid-cols-2">
                              <AdminFormField label="必需标记">
                                <textarea
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, requiredFlagsText: event.target.value },
                                  }))}
                                  className={adminTextareaClassName}
                                />
                              </AdminFormField>
                              <AdminFormField label="屏蔽标记">
                                <textarea
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, blockedFlagsText: event.target.value },
                                  }))}
                                  className={adminTextareaClassName}
                                />
                              </AdminFormField>
                            ...
                            <AdminFormField label="节点状态变更 JSON">
                              <textarea
                                ...
                                onChange={(event) => setNodeForms((current) => ({
                                  ...current,
                                  [selectedNode.id]: { ...selectedNodeForm, stateEffectsText: event.target.value },
                                }))}
                                className={adminTextareaClassName}
                              />
                            </AdminFormField>
                            <div className="flex flex-wrap gap-4">
                              <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, isEnding: event.target.checked },
                                  }))}
                                  className={adminCheckboxClassName}
                                />
                                结局节点
                              ...
                              <label className="inline-flex items-center gap-2 text-sm text-slate-700">
                                <input
                                  ...
                                  onChange={(event) => setNodeForms((current) => ({
                                    ...current,
                                    [selectedNode.id]: { ...selectedNodeForm, aiEnabled: event.target.checked },
                                  }))}
                                  className={adminCheckboxClassName}
                                />
                                启用 AI
                      ...
                      <div className="rounded-[24px] border border-[color:var(--gush-border)] bg-white p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <h3 className="text-sm font-semibold text-slate-950">分支编辑器</h3>
                            <p className="mt-1 text-sm leading-6 text-slate-600">
                        ...
                        {!selectedNode ? (
                          <p className="mt-4 text-sm leading-6 text-slate-500">先选中一个节点，再配置它的选项。</p>
                          ...
                          <div className="mt-4 space-y-4">
                            {(selectedNode.choices || []).map((choice) => {
                              ...
                              return (
                                <div key={choice.id} className="rounded-[22px] border border-[color:var(--gush-border)] p-4">
                                  <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                      <p className="text-sm font-semibold text-slate-950">{form.label || "未命名选项"}</p>
                                      <p className="mt-1 text-xs text-slate-500">{form.choiceKey || "还没填写选项 Key"}</p>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                  ...
                                  <div className="grid gap-4">
                                    <div className="grid gap-4 md:grid-cols-2">
                                      <AdminFormField label="选项 Key">
                                        <input
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, choiceKey: event.target.value },
                                          }))}
                                          className={adminInputClassName}
                                        />
                                      </AdminFormField>
                                      <AdminFormField label="按钮文案">
                                        <input
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, label: event.target.value },
                                          }))}
                                          className={adminInputClassName}
                                        />
                                      </AdminFormField>
                                      <AdminFormField label="跳转节点">
                                        <select
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, targetNodeId: event.target.value },
                                          }))}
                                      ...
                                      <AdminFormField label="排序">
                                        <input
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, sortOrder: event.target.value },
                                          }))}
                                          className={adminInputClassName}
                                        />
                                      </AdminFormField>
                                    ...
                                    <AdminFormField label="选项说明">
                                      <textarea
                                        ...
                                        onChange={(event) => setChoiceForms((current) => ({
                                          ...current,
                                          [choice.id]: { ...form, description: event.target.value },
                                        }))}
                                        className={adminTextareaClassName}
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                      <AdminFormField label="必需标记">
                                        <textarea
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, requiredFlagsText: event.target.value },
                                          }))}
                                          className={adminTextareaClassName}
                                        />
                                      </AdminFormField>
                                      <AdminFormField label="屏蔽标记">
                                        <textarea
                                          ...
                                          onChange={(event) => setChoiceForms((current) => ({
                                            ...current,
                                            [choice.id]: { ...form, blockedFlagsText: event.target.value },
                                          }))}
                                          className={adminTextareaClassName}
                                        />
                                      </AdminFormField>
                                    ...
                                    <AdminFormField label="选项状态变更 JSON">
                                      <textarea
                                        ...
                                        onChange={(event) => setChoiceForms((current) => ({
                                          ...current,
                                          [choice.id]: { ...form, stateEffectsText: event.target.value },
                                        }))}
                                        className={adminTextareaClassName}
                                      />
                                    </AdminFormField>
                                    <div className="flex gap-2">
                                      ...
                                      {form.targetNodeId ? (
                                        <Button size="sm" variant="outline" onClick={() => setSelectedNodeId(form.targetNodeId)}>
                            ...
                            <div className="rounded-[22px] border border-dashed border-[color:var(--gush-border)] p-4">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <p className="text-sm font-semibold text-slate-950">新增选项</p>
                                  <p className="mt-1 text-sm leading-6 text-slate-600">在这里补一个新的分支入口。</p>
                                </div>
                              ...
                              <div className="mt-4 grid gap-4">
                                <div className="grid gap-4 md:grid-cols-2">
                                  <AdminFormField label="选项 Key">
                                    <input
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).choiceKey}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                  ...
                                  <AdminFormField label="按钮文案">
                                    <input
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).label}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                          ...(current[selectedNode.id] || emptyChoice()),
                                  ...
                                  <AdminFormField label="跳转节点">
                                    <select
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).targetNodeId || ""}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                          ...(current[selectedNode.id] || emptyChoice()),
                                  ...
                                  <AdminFormField label="排序">
                                    <input
                                      type="number"
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).sortOrder}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                          ...(current[selectedNode.id] || emptyChoice()),
                                ...
                                <AdminFormField label="选项说明">
                                  <textarea
                                    rows={3}
                                    value={(newChoiceByNode[selectedNode.id] || emptyChoice()).description}
                                    onChange={(event) => setNewChoiceByNode((current) => ({
                                      ...current,
                                      [selectedNode.id]: {
                                        ...(current[selectedNode.id] || emptyChoice()),
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                  <AdminFormField label="必需标记">
                                    <textarea
                                      rows={3}
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).requiredFlagsText}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                          ...(current[selectedNode.id] || emptyChoice()),
                                  ...
                                  <AdminFormField label="屏蔽标记">
                                    <textarea
                                      rows={3}
                                      value={(newChoiceByNode[selectedNode.id] || emptyChoice()).blockedFlagsText}
                                      onChange={(event) => setNewChoiceByNode((current) => ({
                                        ...current,
                                        [selectedNode.id]: {
                                          ...(current[selectedNode.id] || emptyChoice()),
                                ...
                                <AdminFormField label="选项状态变更 JSON">
                                  <textarea
                                    rows={4}
                                    value={(newChoiceByNode[selectedNode.id] || emptyChoice()).stateEffectsText}
                                    onChange={(event) => setNewChoiceByNode((current) => ({
                                      ...current,
                                      [selectedNode.id]: {
                                        ...(current[selectedNode.id] || emptyChoice()),
