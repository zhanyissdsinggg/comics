...
export default function AdminDashboardClean() {
    ...
    async (mode = "initial") => {
      ...
      try {
        const [
          ...
          apiGet(
            "/api/admin/support?page=1&pageSize=5&sortBy=updatedAt&sortOrder=desc",
          ),
        ...
        setWorkspace({
          stats: statsResponse.ok
            ...
            : EMPTY_WORKSPACE.stats,
          series: seriesResponse.ok
            ? pickArray(seriesResponse.data, ["series", "items"])
            : [],
          support: supportResponse.ok
            ? pickArray(supportResponse.data, ["support", "tickets"])
            : [],
          orders: ordersResponse.ok
            ? pickArray(ordersResponse.data, ["orders", "items"])
            : [],
          comments: commentsResponse.ok
            ? pickArray(commentsResponse.data, ["comments", "items"])
            : [],
        });
  ...
  const insights = useMemo(
    () => buildInsights(workspace.series),
    [workspace.series],
  );
