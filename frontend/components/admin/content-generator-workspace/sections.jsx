...
export function GeneratorSummaryCards({
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <AdminFormField label="每种类型作品数" helperText="范围：1 到 20。">
              <input
                ...
                onChange={(event) =>
                  onUpdateField("seriesPerType", event.target.value)
                }
                inputMode="numeric"
                className={adminInputClassName}
              />
            ...
            <AdminFormField label="最少章节数" helperText="范围：1 到 30。">
              <input
                ...
                onChange={(event) =>
                  onUpdateField("minEpisodes", event.target.value)
                }
                inputMode="numeric"
                className={adminInputClassName}
              />
            ...
            <AdminFormField label="最多章节数" helperText="范围：1 到 30。">
              <input
                ...
                onChange={(event) =>
                  onUpdateField("maxEpisodes", event.target.value)
                }
                inputMode="numeric"
                className={adminInputClassName}
              />
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <AdminMetricCard
          label="漫画作品"
          value={String(result.comicsCount ?? 0)}
          detail="本轮生成出的漫画条目数。"
        />
        <AdminMetricCard
          label="小说作品"
          value={String(result.novelsCount ?? 0)}
          detail="本轮生成出的小说条目数。"
        />
        <AdminMetricCard
          label="总章节数"
          value={String(result.totalEpisodes ?? 0)}
          detail="这轮生成覆盖到的章节总量。"
        />
        <AdminMetricCard
          label="耗时"
          value={`${result.duration ?? 0} 秒`}
          detail="执行耗时。"
        />
        <AdminMetricCard
          label="种子"
