...
export function buildSlotOptimizationPlan(slot, replacementCandidates) {
  ...
  if (!slot?.aligned) {
    return {
      ...
      tone: "amber",
      title: "当前配置和方案不一致",
      detail: "先把推荐位对齐到当前编排方案，别让前台和后台判断脱节。",
      actionType: "apply",
      actionLabel: "同步当前建议",
      actionIds: Array.isArray(slot?.recommendedIds) ? slot.recommendedIds : [],
      replacementCandidates,
  ...
  if (weakestEntry && !weakestEntry.readiness.isReady) {
    return {
      ...
      tone: "amber",
      title: "当前作品资料还没补稳",
      detail: `${weakestEntry.series.title} 仍缺 ${weakestEntry.readiness.topIssues.join("、")}。先把作品页补稳，再期待推荐位替它拉表现。`,
      actionType: "edit",
      actionLabel: "去补作品资料",
  ...
  if (!performanceLoaded) {
    return {
      ...
      tone: "cyan",
      title: "表现数据还在回传",
      detail: "推荐位已经上线，先等数据回齐，再判断要不要换内容。",
      actionType: "review",
      actionLabel: "等待数据",
      actionIds: [],
      replacementCandidates,
