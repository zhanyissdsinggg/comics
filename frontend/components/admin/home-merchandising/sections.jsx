  ...
  return (
    <SurfacePanel appearance="light" accent="blue" className="space-y-5">
      ...
      {trackedCurrentSlots.length === 0 ? (
        <EmptyState
          title="先完成推荐位配置"
          description="关键入口上线后，这里才会出现真实表现数据。"
        />
      ) : performanceLoading ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <MetricCard
              ...
              hint="点击后的被跟踪动作。"
              tone={
                performanceSummary.totalConversions > 0 ? "emerald" : "amber"
              }
            />
            <MetricCard
              label="点击率"
              value={formatPercentValue(summaryCtr)}
              hint={`转化率 ${formatPercentValue(summaryConversionRate)}`}
              tone={
                summaryCtr >= 2 ? "emerald" : summaryCtr > 0 ? "amber" : "rose"
              }
            />
          </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {slotPerformanceCards.map((slot) => {
              ...
              const linkedTitles = slot.currentSeries
                .slice(0, 2)
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${getToneClasses(performanceState.tone)}`}
                    >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    <MiniMetric
                      ...
                      value={formatCompactNumber(
                        slot.performance.totalImpressions,
                      )}
                    />
                    <MiniMetric
                      label="点击"
                      value={formatCompactNumber(slot.performance.totalClicks)}
                    />
                    <MiniMetric
                      label="转化"
                      value={formatCompactNumber(
                        slot.performance.totalConversions,
                      )}
                    />
                    <MiniMetric
                      label="点击率"
                      value={formatPercentValue(slot.performance.avgCtr)}
                    />
                    <MiniMetric
                      label="转化率"
                      value={formatPercentValue(
                        slot.performance.avgConversionRate,
                      )}
                    />
                    <MiniMetric
                      label="推荐位编号"
        ...
        {slotOptimizationCards.map((slot) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${getToneClasses(slot.plan.tone)}`}
              >
                {slot.plan.title}
              </span>
            </div>
            ...
            {slot.current ? (
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <MiniMetric
                  label="曝光"
                  value={formatCompactNumber(slot.performance.totalImpressions)}
                />
                <MiniMetric
                  label="点击率"
                  value={formatPercentValue(slot.performance.avgCtr)}
                ...
                <MiniMetric
                  label="转化率"
                  value={formatPercentValue(slot.performance.avgConversionRate)}
                />
              </div>
            ) : null}
            ...
            <div className={`mt-5 ${actionTrayClassName}`}>
              {slot.plan.actionType === "apply" ? (
                <ActionButton
                  ...
                  disabled={
                    savingSlot === slot.id || !slot.canApplyRecommendation
                  }
                >
                  <RefreshCw
                    className={cn(
              ...
              {slot.plan.actionType === "edit" && slot.plan.actionSeriesId ? (
                <ActionButton
                  onClick={() => openSeriesEditor(slot.plan.actionSeriesId)}
                >
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {heroCandidates.map(({ series, score, reasons }) => {
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-[1.25rem] font-semibold tracking-tight text-slate-950">
                    {series.title}
                  </h3>
                  <span
                    className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${getToneClasses(readiness.tone)}`}
                  >
                ...
                <p className="mt-3 text-sm leading-6 text-slate-600">
                  {creatorName ? `署名：${creatorName}` : "署名待补"} |{" "}
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  ...
                  <MiniMetric
                    label="内容基础"
                    value={formatCompactNumber(getReaderProof(series))}
                  />
                ...
                <div className={`mt-5 ${actionTrayClassName}`}>
                  ...
                  <ActionButton
                    onClick={() =>
                      void handleCopyIds(series.title, [series.id])
                    }
                  >
                    <Copy className="h-4 w-4" />
                    复制作品编号
