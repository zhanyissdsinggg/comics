...
function normalize(payload) {
  ...
  return {
    ...
    top: {
      ...
      byRevenue: safeArray(root.topSeries?.byRevenue ?? root.rankings?.byRevenue ?? root.byRevenue, DEFAULT_TOP.byRevenue),
    },
    updates: safeArray(root.recentUpdates ?? root.latestSeries ?? root.updatedSeries, DEFAULT_UPDATES),
    activity: safeArray(root.recentActivities ?? root.activities ?? root.timeline, DEFAULT_ACTIVITY).map((item, index) => ({
      id: item?.id || `activity-${index}`,
      user: item?.user || item?.username || item?.operator || DEFAULT_ACTIVITY[index % DEFAULT_ACTIVITY.length].user,
      action: item?.action || item?.description || item?.message || DEFAULT_ACTIVITY[index % DEFAULT_ACTIVITY.length].action,
      time: item?.time || (item?.createdAt ? relativeTime(item.createdAt) : DEFAULT_ACTIVITY[index % DEFAULT_ACTIVITY.length].time),
    })),
  };
}
  ...
  return (
    <div className="rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6 backdrop-blur-2xl shadow-ios">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className={`flex items-center gap-1 text-sm font-bold ${up ? "text-ios-green" : "text-ios-red"}`}>
          <Trend size={16} />
  ...
  return (
    <div className="rounded-4xl bg-neutral-950/40 p-4">
      <div className="flex items-center gap-2">
        {eyebrow ? <span className="rounded-full bg-ios-green/10 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-ios-green">{eyebrow}</span> : null}
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setData(normalize(response.data));
      } catch (error) {
        console.error("admin dashboard load failed:", error);
        if (!cancelled) {
          setData({ stats: DEFAULT_STATS, top: DEFAULT_TOP, updates: DEFAULT_UPDATES, activity: DEFAULT_ACTIVITY });
        }
      } finally {
    ...
    <div className="space-y-6">
      <section className="rounded-5xl border border-ios-gray-800 bg-gradient-to-br from-ios-green/10 to-emerald-600/5 p-6 backdrop-blur-2xl shadow-ios">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-2xl font-bold text-white"><TrendingUp size={24} className="text-ios-green" />运营总览</h2>
            <p className="mt-2 text-sm text-ios-gray-400">在一个面板里查看内容产出、收入表现和用户增长趋势。</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={refresh} disabled={refreshing} className="flex items-center gap-2 rounded-4xl border border-ios-green/20 bg-ios-green/5 px-4 py-2.5 text-xs font-bold text-ios-green transition hover:bg-ios-green/10 disabled:opacity-50">
              <RefreshCw size={14} className={refreshing ? "animate-spin" : ""} />
              <span>{refreshing ? "刷新中..." : "刷新"}</span>
            </button>
            <button type="button" onClick={exportCsv} className="flex items-center gap-2 rounded-4xl border border-ios-blue/20 bg-ios-blue/5 px-4 py-2.5 text-xs font-bold text-ios-blue transition hover:bg-ios-blue/10">
              <Download size={14} />
              <span>导出</span>
            </button>
            {[
              ...
              <button key={value} type="button" onClick={() => setRange(value)} className={`rounded-4xl px-4 py-2 text-sm font-bold transition ${range === value ? "bg-ios-green text-white shadow-ios" : "bg-neutral-800/50 text-ios-gray-300 hover:bg-neutral-800 hover:text-ios-green"}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        {range === "custom" ? (
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <label className="flex items-center gap-2 text-sm text-ios-gray-400">开始 <input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="rounded-3xl border border-ios-green/20 bg-neutral-800/50 px-4 py-2 text-sm text-white" /></label>
            <label className="flex items-center gap-2 text-sm text-ios-gray-400">结束 <input type="date" value={to} onChange={(event) => setTo(event.target.value)} className="rounded-3xl border border-ios-green/20 bg-neutral-800/50 px-4 py-2 text-sm text-white" /></label>
          </div>
        ) : null}
      </section>
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((item) => <div key={item} className="h-36 animate-pulse rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6" />)}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              <a href="/admin/series" className="rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6 text-center text-white"><BookOpen className="mx-auto mb-3 text-ios-blue" />新增作品</a>
              <a href="/admin/promotions" className="rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6 text-center text-white"><Megaphone className="mx-auto mb-3 text-ios-orange" />创建活动</a>
              <a href="/admin/users" className="rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6 text-center text-white"><Users className="mx-auto mb-3 text-ios-purple" />管理用户</a>
              <a href="/admin/orders" className="rounded-5xl border border-ios-gray-800 bg-neutral-900/50 p-6 text-center text-white"><Receipt className="mx-auto mb-3 text-ios-green" />查看订单</a>
            </div>
          </div>

          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Panel title="浏览量最高作品">
              {data.top.byViews.map((series, index) => <ListRow key={series.id || index} eyebrow={series.type === "comic" ? "漫画" : "小说"} title={series.title} meta={`${number.format(safeNumber(series.views))} 次浏览`} />)}
            </Panel>
            <Panel title="收入最高作品">
              {data.top.byRevenue.map((series, index) => <ListRow key={series.id || index} eyebrow={series.type === "comic" ? "漫画" : "小说"} title={series.title} meta={money.format(safeNumber(series.revenue))} />)}
            </Panel>
          </div>

          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Panel title="最近更新">
              {data.updates.map((series, index) => <ListRow key={series.id || index} eyebrow={series.type === "comic" ? "漫画" : "小说"} title={series.title} meta={`${number.format(safeNumber(series.episodeCount))} 章 · ${relativeTime(series.updatedAt)}`} />)}
            </Panel>
            <Panel title="最近活动" action={<a href="/admin/tracking" className="text-sm font-medium text-ios-green hover:text-emerald-300">查看追踪 &gt;</a>}>
              {data.activity.map((entry, index) => <ListRow key={entry.id || index} title={`${entry.user} ${entry.action}`} meta={entry.time} />)}
            </Panel>
          </div>
        </>
      )}
