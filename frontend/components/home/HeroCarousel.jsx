...
export default function HeroCarousel({ items }) {
  ...
  if (safeItems.length === 0) {
    return (
      <section className="group relative overflow-hidden rounded-[28px] bg-gradient-to-br from-neutral-900/80 to-neutral-950/80 backdrop-blur-xl border border-white/5 p-8 shadow-2xl shadow-black/20">
        ...
        <div className="relative z-10 text-center">
          <p className="text-base text-neutral-400">No featured items.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section
      className="group relative overflow-hidden rounded-[28px] bg-gradient-to-br from-neutral-900/80 to-neutral-950/80 backdrop-blur-xl border border-white/5 shadow-2xl shadow-black/20 transition-all duration-500 hover:scale-[1.01] hover:shadow-emerald-500/10"
    ...
    >
      ...
      <div className="relative z-10 p-3 md:p-4">
        <div
          className="relative aspect-[16/7] overflow-hidden rounded-[24px] sm:aspect-[16/6] md:aspect-[16/5]"
        ...
        >
          ...
          {/* 老王注释：标题和描述 - iOS风格字体优化 */}
          {active?.title && (
            <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
              <h2 className="mb-1 text-xl font-semibold text-white tracking-tight drop-shadow-lg md:mb-2 md:text-3xl">
                {active.title}
              </h2>
              {active.description && (
                <p className="text-xs text-neutral-200 drop-shadow-md md:text-sm leading-relaxed">
                  {active.description}
                </p>
              )}
            </div>
          ...
          {safeItems.length > 1 && (
            <div className="absolute bottom-3 left-1/2 flex w-[90%] max-w-md -translate-x-1/2 gap-1.5 md:bottom-4 md:gap-2">
              {safeItems.map((_, i) => (
                ...
                >
                  {/* 老王注释：iOS风格背景轨道 */}
                  <div className="h-1 w-full overflow-hidden rounded-full bg-white/10 backdrop-blur-md">
                    {/* 老王注释：iOS风格进度条 - 使用emerald渐变 */}
                    <div
                  ...
                  {/* 老王注释：iOS风格悬停提示 */}
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-[12px] bg-neutral-900/90 backdrop-blur-xl border border-white/5 px-3 py-1.5 text-xs text-white opacity-0 shadow-lg shadow-black/20 transition-all duration-300 group-hover/indicator:opacity-100">
