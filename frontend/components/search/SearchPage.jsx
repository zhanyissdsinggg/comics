...
export default function SearchPage() {
  ...
  const searchCommandCards = useMemo(() => {
    const leadSuggestion = suggestions[0] || "";
    const recentQuery = history[0] || "";
    const leadHotKeyword = hotKeywords[0] || keywords[0] || null;
    const trendingLabel = leadHotKeyword?.label || "Romance";
    const trendingValue = leadHotKeyword?.value || trendingLabel;

    return [
      {
        id: "refine-search",
        eyebrow: leadSuggestion
          ? "Smart refine"
          : activeFilterCount > 0
            ? "Live filters"
            : query
              ? "Search flow"
              : "Start broad",
        title: leadSuggestion
          ? `Try "${leadSuggestion}" to tighten the next click.`
          : activeFilterCount > 0
            ? `${activeFilterCount} live filter${activeFilterCount === 1 ? "" : "s"} are shaping this view.`
            : query
              ? "Open the best match first, then widen only if you need to."
              : "A strong search page should make the first move obvious.",
        description: leadSuggestion
          ? "Smart suggestions keep readers moving without forcing them to rewrite the whole query."
          : activeFilterCount > 0
            ? "Type, status, genre, and sort should feel like a control desk instead of a buried settings drawer."
            : query
              ? "When readers already know what they want, search should feel fast, clear, and low-friction."
              : "Before anyone types, discovery should still feel guided instead of empty.",
        ctaLabel: leadSuggestion
          ? `Search ${leadSuggestion}`
          : activeFilterCount > 0
            ? "Open advanced filters"
            : query
              ? "Review filters"
              : "Browse popular now",
        onClick: leadSuggestion
          ? () => updateParam("q", leadSuggestion)
          : activeFilterCount > 0
      ...
      {
        id: "history-trend",
        eyebrow: recentQuery ? "Recent memory" : "Trending now",
        title: recentQuery
          ? `Jump back into "${recentQuery}" without retyping it.`
          : `${trendingLabel} is a live search trend worth checking.`,
        description: recentQuery
          ? "Search history should speed readers back to intent, not just sit in a hidden dropdown."
          : "When readers are still browsing, a live trend is one of the fastest ways to find a strong first series.",
        ctaLabel: recentQuery ? `Search ${recentQuery}` : `Search ${trendingLabel}`,
        onClick: () => updateParam("q", recentQuery || trendingValue),
        accentClass:
          "border-white/10 bg-white/[0.04] text-neutral-100 hover:border-white/20 hover:bg-white/[0.08]",
      },
      {
        ...
        title: query
          ? "If the title is fuzzy, search can still hand you off to creators."
          : "Browse creators when you remember the artist, not the title.",
        description:
          "Top comic storefronts let discovery start from creator identity, studio recognition, and catalog reputation.",
      ...
      {
        id: "chart-route",
        eyebrow: "Chart handoff",
        title: "When search feels cold, the weekly chart should stay one tap away.",
        description: query
          ? "A narrow or awkward search should hand readers off to ranked discovery instead of ending in a dead spot."
          : "The chart is the cleanest bridge from casual browsing into a confident first click.",
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-2 font-display text-2xl font-semibold tracking-tight text-white sm:text-3xl">
                Keep discovery moving before the result grid.
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-neutral-400">
                Good search UX is not just a list of matches. It should help readers refine, recover, and branch into
                the right browse path without friction.
              </p>
            </div>
            <p className="text-sm text-neutral-500">
              {query ? "Built around the current query" : "Built for first-click discovery"}
            </p>
          </div>
          <StorefrontPathwaysGrid cards={searchCommandCards} />
        </SurfacePanel>
