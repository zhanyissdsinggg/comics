...
export default function ReaderPage({ seriesId, episodeId }) {
  ...
  return (
    <main
      className={`min-h-screen bg-[linear-gradient(180deg,#0b0910_0%,#110d16_36%,#0f0d13_100%)] ${nightMode ? "text-neutral-100" : "text-white"}`}
    >
      <ReaderTopBar
      ...
      <div
        className="mx-auto max-w-[1040px] px-0 sm:px-3"
      ...
      {showPaywall ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-[rgba(7,6,12,0.7)] px-4 backdrop-blur-md">
          <div className="w-full max-w-lg rounded-[30px] border border-white/12 bg-[linear-gradient(180deg,rgba(21,18,29,0.98)_0%,rgba(12,10,18,0.98)_100%)] p-6 text-center text-white shadow-[0_28px_80px_rgba(0,0,0,0.42)]">
              ...
              <div className="mb-4 rounded-[22px] border border-white/12 bg-white/[0.04] px-4 py-4 text-left shadow-[0_14px_34px_rgba(0,0,0,0.24)]">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-2 font-display text-[1.1rem] font-semibold tracking-[-0.03em] text-white">
            ...
            {currentPricing.appliedDailyFree ? (
              <p className="mt-3 text-xs font-black uppercase tracking-[0.08em] text-sky-700">
                Free now
              </p>
            ) : currentPricing.discountPct ? (
              <p className="mt-3 text-xs font-black uppercase tracking-[0.08em] text-sky-700">
              ...
              <div className={`mt-4 ${lightInfoCardClass} text-left`}>
                ...
                <div className="mt-3 space-y-2">
                  {upcomingEpisodes.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center justify-between gap-3 rounded-[18px] border border-black/8 bg-[#f3f4f6] px-3 py-3 text-sm shadow-none"
                    >
                      <div>
            ...
            <button
              ...
              className={`mt-6 w-full min-h-[44px] ${lightPrimaryButtonClass} ${
                commerceNotice
                  ? "shadow-[0_0_0_4px_rgba(15,23,42,0.08),0_12px_28px_rgba(15,23,42,0.16)]"
                  : ""
              }`}
            ...
            <button
              ...
              className={`mt-3 w-full ${lightSecondaryButtonClass}`}
            >
