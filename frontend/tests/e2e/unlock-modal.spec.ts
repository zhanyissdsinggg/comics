  ...
  test("series episode lock should route low-balance readers into the store preview when checkout is disabled", async ({
    ...
    await page.route("**/api/**", async (route) => {
      ...
      if (pathname === "/api/auth/me") {
        await fulfillJson(route, {
          user: { id: "user-locked", email: "reader@example.com" },
        });
        return;
      }
      ...
      if (
        ...
        searchParams.get("seriesId") === "series-locked"
      ) {
        await fulfillJson(route, {
          entitlement: {
            seriesId: "series-locked",
    ...
    await page
      .getByRole("heading", { name: "Midnight Static" })
      .waitFor({ state: "visible" });
    await expect(
      page.getByRole("img", { name: "Comic cover image for Midnight Static" }),
    ).toBeVisible();

    const episodeRow = page.locator("#episode-series-locked-e2");
    const unlockButton = episodeRow.getByRole("button", {
      name: "Unlock with Points",
    });
    ...
    await expect(
      dialog.getByText("Unlock Chapter 2 for 15 Points"),
    ).toBeVisible();
