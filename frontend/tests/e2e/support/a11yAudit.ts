...
export async function expectNoBasicA11yAuditIssues(
  ...
  const result = await page.evaluate<A11yAuditResult, BasicA11yAuditOptions>(
    (auditOptions) => {
      ...
      const hasReadableText = (value: string) =>
        /[A-Za-z\u00C0-\u024F\u4E00-\u9FFF]/.test(value);
      ...
      const isIgnoredByClosestSelector = (
        element: Element,
        selectors: string[] = [],
      ) =>
      ...
      const missingImageAlt = Array.from(document.querySelectorAll("img"))
        ...
        .filter(
          (image) =>
            !isIgnoredByClosestSelector(image, auditOptions.ignoreImagesInside),
        )
        .map((image) => ({
      ...
      const unnamedIconControls = Array.from(
        ...
        .filter(
          (control) =>
            !isIgnoredByClosestSelector(
              control,
              auditOptions.ignoreControlsInside,
            ),
        )
