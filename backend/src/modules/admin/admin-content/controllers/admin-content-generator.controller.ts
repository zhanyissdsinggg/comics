...
const COMIC_SEEDS: SeriesSeed[] = [
  {
    ...
    description: "Two rivals become unwilling partners after a supernatural contract binds their futures.",
    genres: ["Romance", "Drama"],
    coverTone: "warm",
  },
  {
    title: "Neon District",
    description: "A courier with a hidden power tears through a lawless cyberpunk city.",
    genres: ["Action", "Sci-Fi"],
    coverTone: "neon",
  },
  {
    title: "Crimson Thesis",
    description: "An elite arts academy hides brutal rivalries behind polished marble walls.",
    genres: ["Drama", "Mystery"],
    coverTone: "crimson",
  },
  {
    title: "Velvet Trigger",
    description: "A noir assassin romance where every mission puts love and loyalty at war.",
  ...
  {
    title: "Skyline Heist",
    description: "A rooftop crew steals impossible secrets from the most secure towers on earth.",
    genres: ["Action", "Adventure"],
    coverTone: "cool",
  },
  {
    title: "Petal Requiem",
    description: "A quiet flower shop becomes the center of an old family curse.",
    genres: ["Fantasy", "Drama"],
    coverTone: "pastel",
  },
  {
    title: "Iron Pulse",
    description: "Teen pilots sync with biomechanical armor to survive a collapsing megacity.",
    genres: ["Sci-Fi", "Action"],
    coverTone: "steel",
  },
  {
    title: "Queens of Ash",
    description: "A palace succession battle turns deadly when prophecy enters the court.",
    genres: ["Fantasy", "Historical"],
    coverTone: "gold",
  },
  {
    title: "Double Exposure",
    description: "A celebrity photographer uncovers a conspiracy hidden inside her own archive.",
...
const NOVEL_SEEDS: SeriesSeed[] = [
  {
    ...
    description: "A disgraced strategist rebuilds power from the ruins of a coastal kingdom.",
    genres: ["Fantasy", "Adventure"],
    coverTone: "sand",
  },
  {
    title: "After the Firewall",
    description: "A rogue analyst escapes a corporate state with the only key to the truth.",
    genres: ["Sci-Fi", "Thriller"],
    coverTone: "cobalt",
  },
  {
    title: "Second Bloom",
    description: "A chef returns home and finds that grief can be rebuilt into family.",
    genres: ["Slice of Life", "Drama"],
    coverTone: "rose",
  },
  {
    title: "The Fourth Witness",
    description: "A courtroom thriller where each testimony rewrites the crime.",
    genres: ["Mystery", "Thriller"],
    coverTone: "slate",
  },
  {
    title: "Winter Meridian",
    description: "Explorers cross a frozen continent to stop a machine buried under the ice.",
    genres: ["Adventure", "Sci-Fi"],
    coverTone: "frost",
  },
  {
    title: "Velour Hearts",
    description: "A luxury hotel romance where secrets cost more than the suites.",
    genres: ["Romance", "Drama"],
    coverTone: "plum",
    adult: true,
  },
  {
    title: "Atlas Breaker",
    description: "An ex-soldier follows an impossible map that keeps changing every night.",
    genres: ["Action", "Adventure"],
    coverTone: "ember",
  },
  {
    title: "House of Lanterns",
    description: "Ghost stories and inheritance battles collide inside a seaside manor.",
    genres: ["Supernatural", "Drama"],
    coverTone: "amber",
  },
  {
    title: "Blue Signal",
    description: "A former pop idol chases a second career through radio and late-night confession.",
    genres: ["Drama", "Music"],
    coverTone: "indigo",
  },
  {
    title: "Ruin Market",
    description: "Merchants, thieves, and relic hunters battle over a city built on ancient machines.",
...
function parsePositiveIntField(fieldName: string, value: unknown, fallback: number): number {
  if (value === undefined) {
    return fallback;
  }
...
function resolveSettings(body: GeneratorRequestBody): GeneratorSettings {
  const seriesPerType = clamp(
    parsePositiveIntField("seriesPerType", body.seriesPerType, DEFAULT_SERIES_PER_TYPE),
    1,
    MAX_SERIES_PER_TYPE,
  );
  const minEpisodes = clamp(
    parsePositiveIntField("minEpisodes", body.minEpisodes, DEFAULT_MIN_EPISODES),
    1,
    DEFAULT_MAX_EPISODES,
  );
  const maxEpisodes = clamp(
    parsePositiveIntField("maxEpisodes", body.maxEpisodes, DEFAULT_MAX_EPISODES),
    minEpisodes,
    DEFAULT_MAX_EPISODES,
  );
...
const GENERATED_CONTENT_DIR = join(process.cwd(), "public", "uploads", "generated", "content");

type AssetRequest = Pick<Request, "protocol" | "headers" | "get"> | undefined;

const TONE_PALETTES: Record<string, { background: string; accent: string; foreground: string }> = {
...
function toPublicAssetPath(absolutePath: string): string {
  return absolutePath.replace(join(process.cwd(), "public"), "").replace(/\\/g, "/");
}

function buildSvgAsset(width: number, height: number, title: string, subtitle: string, tone: string): string {
...
function writeGeneratedAsset(relativeSegments: string[], fileName: string, fileContent: string): string {
  const absolutePath = join(GENERATED_CONTENT_DIR, ...relativeSegments, fileName);
...
function buildComicPages(
  ...
  return Array.from({ length: pageCount }, (_, index) => {
    ...
    const relativePath = writeGeneratedAsset(
      ...
      buildSvgAsset(800, 1200, `Episode ${pageNumber}`, `${seriesId} · ${episodeId}`, tone),
    );
...
function buildNovelParagraphs(seed: SeriesSeed, episodeNumber: number): string[] {
  const fragments = [
...
export class AdminContentGeneratorController {
  ...
  @Post()
  async generate(@Body() body: GeneratorRequestBody | unknown = {}, @Req() req?: Request) {
    ...
    const seedSource = requestBody.seed !== undefined ? String(requestBody.seed) : new Date().toISOString();
    ...
    return {
      ...
      totalEpisodes: comics.episodeRecords.length + novels.episodeRecords.length,
  ...
  private buildBatch(input: {
    ...
    for (let index = 0; index < input.settings.seriesPerType; index += 1) {
      ...
      const episodeCount = randomInt(input.random, input.settings.minEpisodes, input.settings.maxEpisodes);
      ...
      for (let episodeNumber = 1; episodeNumber <= episodeCount; episodeNumber += 1) {
        ...
        const releasedAt = new Date(Date.now() - (episodeCount - episodeNumber) * DAY_MS);
        const isFree = input.type === "comic" ? episodeNumber <= 3 : episodeNumber <= 5;

        episodeRecords.push({
          id: episodeId,
          seriesId,
          title: input.type === "comic" ? `Episode ${episodeNumber}` : `Chapter ${episodeNumber}`,
          ...
          pages:
            input.type === "comic"
              ? buildComicPages(seriesId, episodeId, randomInt(input.random, 10, 16), seed.coverTone, input.request)
              : [],
          paragraphs: input.type === "novel" ? buildNovelParagraphs(seed, episodeNumber) : [],
        });
      }
    }
