...
describe('AdminContentGeneratorController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<AdminContentGeneratorController>(AdminContentGeneratorController);
  ...
  it('rejects malformed numeric fields instead of silently coercing them', async () => {
    ...
    ).rejects.toThrow(new BadRequestException('seriesPerType must be a positive integer.'));
  });
  ...
  it('writes local generated asset urls instead of external placeholder urls', async () => {
    ...
    const seriesCall = (prisma.series.createMany as jest.Mock).mock.calls.at(-1)?.[0];
    const episodeCall = (prisma.episode.createMany as jest.Mock).mock.calls.at(-1)?.[0];
    const comicSeries = seriesCall.data.find((item: Record<string, unknown>) => item.type === 'comic');
    const comicEpisode = episodeCall.data.find(
      (item: Record<string, unknown>) => item.seriesId === comicSeries.id && Array.isArray(item.pages) && item.pages.length > 0,
    );

    expect(comicSeries.coverUrl).toMatch(/^\/uploads\/generated\/content\/comic-/);
    expect(comicEpisode.pages[0].url).toMatch(/^\/uploads\/generated\/content\/comic-/);
  ...
  it('keeps an explicit empty seed deterministic across runs', async () => {
    ...
    const captureSeries = async (iso: string) => {
      ...
      const call = (prisma.series.createMany as jest.Mock).mock.calls.at(-1)?.[0];
      return call.data.map(({ id, latestEpisodeId, coverUrl, ...rest }: Record<string, unknown>) => rest);
    };
